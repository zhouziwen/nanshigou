...
public class AddressAddActivity extends AppCompatActivity {
    ...
    private void addAddress() {
        ...
        Constant.mFinalHttp.post(Constant.LINK_MOBILE_ADDRESS_ADD, ajaxParams, new AjaxCallBack<Object>() {
            @Override
            public void onSuccess(Object o) {
                super.onSuccess(o);
                if (TextUtil.isNcJson(o.toString())) {
                    try {
                        ...
                        if (datas.contains("失败")) {
                            ToastUtil.show(mActivity, "添加失败，请重试");
                        } else {
                            ToastUtil.show(mActivity, "添加成功");
